        ...
        <div id="add" class="w-75 mx-auto border p-3 rounded">
            <b-form @submit.prevent="addEmpl">
                <div class="row form-group">
                    ...
                    <select v-model="form.newLeaderId" @change="selectUser">
...
export default {
    ...
    methods: {
        getUsers() {
            axios
                .get('/api/project/membersForLeader', {
                    headers: {
        ...
        selectUser() {
            console.log('Выбран пользователь с ID:', this.form.newLeaderId);
        },

        addEmpl() {
            let data = {
                newLeaderId: this.form.newLeaderId
            };
            if (this.form.newLeaderId) {
                axios
                    .patch("/api/project/addProjectLeader", data, {
                        headers: {
                            'authorization': `Bearer ${localStorage.access_token}`,
                            'projectid': localStorage.proj_id
                        }
